...
export default function VideoModal({ isOpen, videoId, onClose }: VideoModalProps) {
    ...
    return (
        <div className={`fixed inset-0 z-[200] flex items-center justify-center p-4 transition-all duration-700 ease-in-out ${isVisible ? 'opacity-100' : 'opacity-0 scale-105'}`}>
            ...
            {/* Content Container */}
            <div className={`relative w-full aspect-[9/16] sm:h-[85vh] sm:w-auto mx-auto bg-black border-0 sm:border sm:border-white/10 shadow-2xl sm:rounded-2xl transition-all duration-700 sm:duration-1000 sm:delay-300 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`}>
                ...
                {videoId && (
                    <div className="relative w-full h-full overflow-hidden bg-black flex items-center justify-center sm:rounded-2xl">
                        <video
                            key={videoId}
                            src={videoId}
                            className="absolute border-0 w-full h-full object-cover"
                            controls
                            autoPlay
                            playsInline
                            preload="metadata"
